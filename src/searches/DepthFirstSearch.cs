    ...
    private static void RecursiveSearch<Gb, M, T>(Gb[] gbs, DFParameters<Gb, M, T> parameters, DFState<M, T> state, HashSet<int> seenStates) where Gb : PokemonGame
        ...
        foreach(Edge<M, T> edge in state.Tile.Edges[state.EdgeSet].OrderBy(x => x.Action != state.LastDir)) { // try the same direction first
            ...
            MultiThread.For(state.IGT.Length, gbs, (gb, f) => {
                ...
                IGTState igt;
                if(prev.State != null) { // we're in the overworld, execute action
            ...
            int totalSuccesses = parameters.EncounterCallback != null ? results.TotalSuccesses : 0;
            ...
            if(totalOverworld > 0 && totalOverworld + totalSuccesses >= parameters.SuccessSS) { // success still possible
                if(parameters.RNGSS <= 0 || results.RNGSuccesses(0x9) >= parameters.RNGSS) {
